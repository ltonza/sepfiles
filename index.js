...
const pathIn = path.resolve(__dirname, "input");
const pathOut = path.resolve(__dirname, "output");
...
fs.readdirSync(pathOut).forEach(file => {
  let filename = path.resolve(pathOut, file);
  console.log(`\tRemovendo: ${filename}`);
  fs.unlinkSync(filename);
});

console.log(`Lendo diretorio: ${pathIn}`);

fs.readdir(pathIn, function(err, files) {
  console.log(`Arquivos encontrados: ${files.length}`);
  files.forEach(file => {
    let filename = path.resolve(pathIn, file);
    ...
    Object.entries(parts.B1).forEach(([key, value]) => {
      let outputFile = path.resolve(
        pathOut,
        file.replace(path.extname(file), `_${key}${path.extname(file)}`)
      );
